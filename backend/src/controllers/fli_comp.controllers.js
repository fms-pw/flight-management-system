...
export async function createAirport(req, res) {
  ...
  if (airports.some(({ name, code, city, country }) => !name || !code || !city || !country)) {
    return res.status(400).json({success:false, message: "Please provide complete airport data." });
  }

  try {
    const result = await Airport.insertMany(airports);
    return res.status(201).json({success:true, message: "Airports inserted successfully", data: result });
  } catch (err) {
    console.error(err, "in airport creation");

    if (err.code === 11000) {
      return res.status(409).json({success:false, message: "Duplicate airport code" });
    }
...
=======
  if (schedules.some(({ flightId, routeId, departureTime, arrivalTime }) => !flightId|| !routeId  || !arrivalTime || !departureTime)) {
    return res.status(400).json({success:false, message: "Please provide complete schedule data." });
>>>>>>> fca5f0c93d2f33608416c23d7172f31d30af6e76
  }

  try {
    const result = await Schedule.insertMany(schedules);
    return res.status(201).json({success:true, message: "Schedules created successfully", data: result });
  } catch (err) {
...
=======
  if (routes.some(({ origin, destination,  routeCode, duration, distance }) => !origin|| !destination ||  !routeCode || !duration || !distance)) {
    return res.status(400).json({success:false, message: "Please provide complete route data." });
>>>>>>> fca5f0c93d2f33608416c23d7172f31d30af6e76
  }

  try {
    const result = await FlightRoute.insertMany(routes);
    return res.status(201).json({success:true, message: "routes created successfully", data: result });
  } catch (err) {
...
=======
  if (seats.some(({ number, Class,  seatType }) => !number || !Class || !seatType)) {
    return res.status(400).json({success:false, message: "Please provide complete seat data." });
>>>>>>> fca5f0c93d2f33608416c23d7172f31d30af6e76
  }

  try {
    const result = await Seat.insertMany(seats);
    return res.status(201).json({success:true, message: "seats created successfully", data: result });
  } catch (err) {
